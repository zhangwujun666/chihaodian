...
<body>
	...
	<div class="pd-20" style="width: 80%">
			...
			<div class="row cl">
				...
				<label class="form-label col-2">卡券ID：</label>
				<div class="formControls col-10">
					<input type="text" id="card_id" disabled="disabled" placeholder="请填写卡券ID" value="" class="input-text" style="width: 80%">
					<button id="creatCard" onclick="creatCard()" style="height: 30px; width: 300px; background-color: red; color: white; font-size: 15px;">添加商品名称后点击添加卡券ID</button>
				</div>
			</div><br>
			<div class="row cl">
	...
	<script type="text/javascript">
		...
		function add(){
			...
			$.ajax({
				...
				type:'post',
				data:'goods_name='+encodeURI(goods_name)+'&card_id='+card_id+'&ctg_id='+ctg_id+'&goods_img='+goods_img+'&goods_spe='+goods_spe
		...
		function upload() {
			...
			$.ajaxFileUpload({
				...
				success : function(rs) //相当于java中try语句块的用
				{
					if (rs != "") {
						$('#img').html("");
